...
def Getmotherbrew_tbl():
    ...
    # Decode the JSON response into a dictionary and use the data
    global number, beername, beertype, Grainprice, Grainweight, GrainType1, GrainType2, GrainType3, customer_id, payment_id, order_id, keg_quantity, keg_volume, keg_price 
...
#     print('Grain Weight: ' + Grainweight)
    ...
    return number, beername, beertype, Grainprice, Grainweight, GrainType1, GrainType2, GrainType3, customer_id, payment_id, order_id, keg_quantity, keg_volume, keg_price
Getmotherbrew_tbl()
...
def Post():
    ...
    # Set the request parameters
    #(THIS works!) url = 'https://emplkasperpsu1.service-now.com/api/now/table/x_snc_beer_brewing_log_table?sysparm_fields=preperation_rest_clean%2Cprep_time_end%2Cpreperation_quality_check%2Cprep_time_start%2Cnumber'
    ...
    # Do the HTTP request
    response = requests.post(url, auth=(user, pwd), headers=headers,
                             data="{\"prep_time_start\":\"" + str(start1) + "\",\"prep_time_end\":\"" + str(end1) + "\",\"total_price\":\"" + total_price + "\",\"preperation_quality_check\":\"true\",\"preperation_rest_clean\":\"true\",\"number\":\"" + str(number) + "\"}")
